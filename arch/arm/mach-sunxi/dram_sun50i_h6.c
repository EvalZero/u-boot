...
static void mctl_set_timing_lpddr3(struct dram_para *para)
{
	...
	writel(0x00000000, &mctl_ctl->dfitmg2);

	writel(0x42c21590, &mctl_phy->ptr[0]);
	writel(0x682b12c0, &mctl_phy->ptr[1]);
	writel(0x00083def, &mctl_phy->ptr[2]);
	writel(0x04b24541, &mctl_phy->ptr[3]);
	writel(0x0ba41ff9, &mctl_phy->ptr[4]);
	writel(0x206ac356, &mctl_phy->ptr[5]);
...
static void mctl_channel_init(struct dram_para *para)
{
	...
	/* TODO: non-LPDDR3 types */
	writel(0x1000b0c7, &mctl_phy->dtcr[0]);
	/* FIXME: This is wrong. */
//	clrbits_le32(&mctl_phy->dtcr[0], 0xfffffff);
//	setbits_le32(&mctl_phy->dtcr[0], 0x10000040);
	if (para->clk <= 792) {
		if (para->clk <= 672) {
			if (para->clk <= 600)
				val = 0x300;
...
/*	clrbits_le32(&mctl_phy->zq[0].zqor[0], 0xff);
	...
	writel(0x3ba3b, &mctl_phy->zq[0].zqor[0]);
/*	clrbits_le32(&mctl_phy->zq[0].zqpr[1], 0xffff);
	...
	writel(0x7705cbf, &mctl_phy->zq[0].zqpr[1]);
...
//	setbits_le32(&mctl_phy->dsgcr, 0xc0);
	clrbits_le32(&mctl_phy->dsgcr, 0x80);
	setbits_le32(&mctl_phy->dsgcr, 0x40);
